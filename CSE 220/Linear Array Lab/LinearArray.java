...
public class LinearArray{
      ...
      public static void reverseArray(int [] source){
            int[] newArray = copyArray(source, source.length);
      ...
      public static int [] rotateLeft(int [] source, int k){
            ...
            int[] newArray = copyArray(source, length);
            for (int i = 0; i < length; ++i) {
                  if (i-k < 0){
                        source[length-k+i] = newArray[i];
                        continue;
                  }
                  source[i-k] = newArray[i];
            }
            return source;   
      }
      ...
      public static boolean insert(int [] arr, int size, int elem, int index){
            ...
            if (zeroIndex != -1) {
                  System.out.printf("Number of elements after insertion: %d\n", size+1);
                  if (arr[index] == 0) {
                        arr[index] = elem;
                        return true;
                  }
                  int[] newArray = copyArray(arr, arr.length);
