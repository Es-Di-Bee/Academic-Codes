...
public class Task2 {
    public static void main (String[] args) {
        ...
        System.out.println ("Enter no. of process and the burst time and\npriority of the processes sequentially:");
        ...
        while(true) {
            int current_process = n, min_priority = 999;
            ...
            for (int i = 0; i < n; i++) {
                if (arrival_time[i] <= sys_time
                        && !completed_flag[i]
                        && priority[i] < min_priority) {
                    current_process = i;
                    min_priority = priority[i]; // always updating the highest priority
                }
            }
